...
- $\color{orange}{AC(q)_{min}}$ at a market price of $6
  - Firm earns .hi[“normal” economic profits] (of 0)
...
- It would seem these firms earn .hi-green[profits] (like Firm 1), as they have lower costs...
...
| **A 1% p-change** | **More than 1%** change in $q_S$ | **1% change** in $q_S$ | **Less than 1%** change in $q_S$ | 

.footnote[Compare to [price elasticity of demand](https://microf21.classes.ryansafner.com/slides/1.7-slides#5)]
---
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  scale_y_continuous(breaks=seq(0,10,1),
                     ...
                     expand=expand_scale(mult=c(0,0.1)),
                     labels = function(x){paste("$", x, sep="")})+
  geom_label_repel(data = tibble(x = 0,
                                 y = 2),
                   aes(x=x, y=y),
                   color="purple",
                   hjust = 0,
                   vjust = 1.5,
                   label=expression(epsilon == infinity))+
  geom_label_repel(data = tibble(x = 2,
                                 y = 3),
                   aes(x=x, y=y),
                   color="purple",
                   hjust = 0,
                   vjust = 1.5,
                   label=expression(epsilon == 3))+
  geom_label_repel(data = tibble(x = 4,
                                 y = 4),
                   aes(x=x, y=y),
                   color="purple",
                   hjust = 0,
  ...
  geom_label_repel(data = tibble(x = 6,
                                 y = 5),
                   aes(x=x, y=y),
                   color="purple",
                   hjust = 0,
                   vjust = 1.5,
                   label=expression(epsilon == 1.67))+
  geom_label_repel(data = tibble(x = 8,
                                 y = 6),
                   aes(x=x, y=y),
                   color="purple",
                   hjust = 0,
                   vjust = 1.5,
                   label=expression(epsilon == 1.5))+
  geom_label_repel(data = tibble(x = 10,
                                 y = 7),
                   aes(x=x, y=y),
                   color="purple",
                   hjust = 0,
                   vjust = 1.5,
                   label=expression(epsilon == 1.4))+
  #coord_cartesian(clip="off")+
  labs(x = "Quantity",
       y = "Price")+
  theme_classic(base_family = "Fira Sans Condensed", base_size=20)
