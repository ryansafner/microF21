...
- The .hi[(opportunity) cost] producers must pay to purchase (scarce) inputs to produce products comes from the fact that **other people** .hi-blue[demand] those same inputs to consume or produce **other things** for other purposes!
  - You are not paying the price just to buy them, but also to **pull them out of other valuable lines of production** in the economy!
]
...
- Producing **Bananas** (x-axis), start with most productive plot first (.green[B]), then start cultivation on (.blue[A])
  - .hi-turquoise[“Intensive margin”]: producing more on existing (most productive) plot (on .green[B])
  - .hi-orange[“Extensive margin”]: bringing new plots into cultivation for production (moving to .blue[A])
]
]
...
```{r}
ppf_jagged+
  annotate("segment", x = 5, xend = 15, y = 5, yend = 5, colour = "black", size=2, alpha=1, arrow=arrow(length=unit(0.5,"cm"), ends="last", type="closed"))
```
...
- Based on his preferences, his productive & consumption .hi[optimum] is point A (highest .green[Indifference curve] tangent to .blue[PPF])
...
home_ppf_1<-ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  geom_segment(x = 0, xend = 2, y = 2.75, yend = 2.75, linetype = "dotted", size = 1)+
  geom_segment(x = 2, xend = 2, y = 2.75, yend = 0, linetype = "dotted", size = 1)+
  geom_label(x = 4, y = home_price_1(4), label = expression(frac(p[b],p[c])), size = 3, color = "green")+
  geom_point(data = home_point,
             mapping=aes(x=x, y=y),
             size = 3)+
  geom_text_repel(data = home_point,
...
2. .hi[Specialization] and .hi[comparative advantage]
